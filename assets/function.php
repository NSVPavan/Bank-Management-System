...
function sendLoan($action,$amount,$bankAccN0)
{
	$con = new mysqli('localhost','root','','mybank');
	if ($action == 'transfer')
	{
		return $con->query("insert into transaction (action,debit,other,userId) values ('transfer','$amount','$bankAccNo','$_SESSION[userId]')");
	}
}
 ?>